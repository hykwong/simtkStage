 ...
 * communityPage.php
 * 
 * File to handle community display.
 *
...
require_once 'communityUtils.php';
...
<?php
		...
		if (session_get_user()) {
			...
			$sql = "SELECT user_id FROM trove_admin " .
				"WHERE trove_cat_id=$1 " .
				"AND user_id=$2";
			$result = db_query_params($sql, array($cat_id, $user_id));
			$cntRows = db_numrows($result);
			db_free_result($result);
		}
		if (forge_check_global_perm('forge_admin') || $cntRows > 0) {
?>
		<div class="share_text">
			<a class="btn-blue share_text_button" 
				href="/category/communityAdmin.php?cat=<?php
				echo $cat_id; ?>" style="font-size:15px;"
				>Community administration</a>
		</div><!-- share_text -->
<?php
		}
?>
...
<div>
<?php
	$arrAdmins = getAdministrators($cat_id);
	for ($cnt = 0; $cnt < count($arrAdmins); $cnt++) {
		// Get user_id.
		$user_id = $arrAdmins[$cnt];
		// Get user object.
		...
		if (!empty($userObj->data_array['picture_file'])) {
			echo $userObj->data_array['picture_file'];
		}
		else {
			echo "user_profile.jpg";
		}
