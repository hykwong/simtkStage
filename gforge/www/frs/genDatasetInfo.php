...
// Dataset description.
$strDataset = genDatasetDesc($cur_group_obj, $thePackages, $lastRelDate, $lastRelId);
echo $strDataset;


// Generate the Dataset description.
function genDatasetDesc($groupObj, $thePackages, &$lastRelDate, &$lastRelId) {
	...
	foreach ($thePackages as $idxPack=>$packageInfo) {
		...
		foreach ($theReleases as $idxRel=>$releaseInfo) {
			...
			if ($relStatus != 1) {
				// Ignore nono-active release. Skip to next release.
				continue;
			}
			...
			if ($relDate > $lastRelDate) {
				...
				$lastRelDate = $relDate;
				// Update the latest release id.
				$lastRelId = $releaseInfo["release_id"];
			}
	...
	if (count($thePackages) >= 1 && $hasFiles !== false) {
		...
		$strHeader = genDatasetHeader($groupObj, $lastRelDate, $lastRelId, $thePackages);
...
function genDatasetHeader($groupObj, $lastRelDate, $lastRelId, $thePackages) {
